...
class tinyAcceleratedEVM(environment.Environment):
    ...
    def __init__(self, gas_paid: int, bytecode: str = "6005600401", debug_mode: bool = False):
        super().__init__()
    ...
    def reset_env(
        ...
        state = EnvState(
            memory=jnp.zeros(params.memory_capacity,dtype=jnp.uint8),
            stack=jnp.zeros((1024,32),dtype=jnp.uint8),
            storage=jnp.zeros((params.storage_capacity,32*2),dtype=jnp.uint8),
            stack_height=0,
            pc=0,
    ...
    def get_obs(self, state: EnvState) -> chex.Array:
        ...
        return jnp.hstack([state.stack, state.storage])
    ...
    def state_space(self, params: EnvParams) -> spaces.Dict:
        ...
        return spaces.Dict(
            {
                "memory": spaces.Box(0,255, params.memory_capacity),
                "memory": spaces.Box(0,255, (params.storage_capacity, 32*2)),
...
def step_machine(action: int, state: EnvState, params: EnvParams, jax_bytecode: chex.Array, opcode_list) -> EnvState:
    ...
    gas, incr,inp_num,out_num,op,memory_op,storage_op = operation_lookup(instruction)
    ...
    stack = jax.lax.select(
        operation_cond,
        stack
            .at[new_sh:s_height,:].set(0)
            .at[new_sh-1,:].set(apply_operation(stack, memory, storage, op, inp_num, out_num, s_height, operation_cond)),
        stack,
    )

    # @@@ Handling operations involving memory @@@ 
    (offset,length,value) = memory_op(stack,memory,inp_num)
    memory = jax.lax.select(
        memory_cond,
        memory.
            at[offset:offset+length].set(value),
        memory,
    )
...
def apply_operation(stack: chex.Array, memory: chex.Array, storage: chex.Array, op, inp_num, out_num, s_height, operation_cond) -> chex.Array:
...
"""
Return the (min_gas, pc_increment,inp_num,out_num,op,memory_op,storage_op)
===============================================================
op - defines how to update stack 
memory_op - defines how to update memory 
storage_op - defines how to update account storage
"""
def operation_lookup(instruction):

    # INSTRUCTIONS 00 - 09
    # Stack direction: -> ... ,x[2] ,x[1], x[0]
    arithmetic_instructions = {
        "0x01" : (3,1,2,1, lambda *x: x[0]+x[1],None,None),
        "0x02" : (5,1,2,1, lambda *x: x[0]*x[1],None,None),
        "0x03" : (3,1,2,1, lambda *x: x[0]-x[1],None,None),
        "0x04" : (5,1,2,1, lambda *x: x[0]//x[1],None,None),
        "0x05" : (5,1,2,1, lambda *x: x[0]//x[1],None,None), # TODO: Implement truncated for SDIV
        "0x06" : (5,1,2,1, lambda *x: x[0]%x[1],None,None),
        "0x07" : (5,1,2,1, lambda *x: x[0]%x[1]* np.sign(x[0]*x[1]),None,None),
        "0x08" : (8,1,3,1, lambda *x: (x[0]+x[1])%x[2],None,None),
        "0x09" : (8,1,3,1, lambda *x: (x[0]*x[1])%x[2],None,None),

        "0x51" : (3,1,1,1, None,None,None), 
    }

    # INSTRUCTIONS 52 - 53 
    # op for these will return new state for memory and stack
    memory_instructions = { 
        "0x52" : (3,1,2,0, lambda *x: 0, lambda *x: (array_to_uint32(x[0]),32,x[1]),None),
        "0x53" : (3,1,2,0, lambda *x: 0, lambda *x: (array_to_uint32(x[0]),32,x[1]),None),
    }

    # INSTRUCTIONS 60 - 7F
    push_instructions = {hex(int("0x60",0)+i):(3,i+2,0,1,None,None,None) for i in range(32)}
...
if __name__=="__main__":
    ...
    while not terminal:
        ...
        _, pc_increment,_,_,_,_,_ = operation_lookup(prev_opcode)
